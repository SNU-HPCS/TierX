#!/bin/bash
# setup_motivation_application.sh
# Set up TierX.yaml for application motivation and run simulation
...
echo "Applying motivation application changes..."

# Update DSE.py data_dir
sed -i "s|data_dir = 'data_DSE'|data_dir = 'data_DSE_motivation_application'|" src/DSE.py
# Update Plot_graph.py data_dir
sed -i "s|data_dir = 'data_DSE'|data_dir = 'data_DSE_motivation_application'|" src/Plot_graph.py
# Update Plot_graph.py result_dir
sed -i "s|result_dir = f'results'|result_dir = f'results_motivation_application'|" src/Plot_graph.py

# Update TierX.yaml sections using helper (preserves other sections like power_constraints, pipelining, etc.)
update_yaml_list TierX.yaml applications "Seizure" "SpikeSorting"
...
echo "Motivation application changes applied."

# Disable LIGHTWEIGHT mode to save graphs
sed -i "s|LIGHTWEIGHT=true|LIGHTWEIGHT=false|" run.sh

echo "Run run.sh for motivation application simulation."
./run.sh
